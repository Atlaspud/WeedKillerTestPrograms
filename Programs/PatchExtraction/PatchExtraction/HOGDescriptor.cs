...
namespace PatchExtraction
{
    class HOGDescriptor
    {
        ...
        public float[] compute(Image<Gray, byte> image)
        {
            return compute(image.Convert<Bgr, byte>());
        }
        ...
        public float[] compute(Image<Bgr, byte> image)
        {
            ...
            Image<Bgr, float> dxImage = image.Convolution(derivativeMasks[0]);
            Image<Bgr, float> dyImage = image.Convolution(derivativeMasks[1]);
            ...
            for (int blockStartY = 0; blockStartY < (height - blockStride); blockStartY += blockStride)
            {
                for (int blockStartX = 0; blockStartX < (width - blockStride); blockStartX += blockStride)
                {
                    int blockEndY = blockStartY + blockSize * cellSize;
                    int blockEndX = blockStartX + blockSize * cellSize;
                    for (int y = blockStartY; y < blockEndY; y++)
        ...
        public Image<Bgr, byte> visualise(Image<Gray, byte> image, float[] descriptor, int scaleFactor)
        {
            return visualise(image.Convert<Bgr, byte>(), descriptor, scaleFactor);
        }
                ...
                for (int cellY = 0; cellY < windowSize / cellSize; cellY++)
                {
                    ...
                    for (int bin = 0; bin < numberOfBins; bin++)
                    {
                        ...
                        if (currentGradientMagnitude > 0)
                        {
                            ...
                            visual.Draw(new LineSegment2D(start,end), new Bgr(0, 0, 255), 2);
                        }
                    }
                }
            }
        ...
        public static ConvolutionKernelF[] getDerivativeMasks(DerivativeMask type)
        {
            ...
            switch (type)
            {
                ...
                case DerivativeMask.RobertsCross2D:
                    ...
                    dxMask[0, 0] =  0; dyMask[0, 0] =  1;
                    dxMask[1, 0] =  1; dyMask[1, 0] =  0;
                    dxMask[0, 1] = -1; dyMask[0, 1] =  0;
                    dxMask[1, 1] =  0; dyMask[1, 1] = -1;
                    break;
